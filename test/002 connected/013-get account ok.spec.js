        ...
        try {
            superAgent
                ...
                .end((error, response) => {
                    ...
                    expect(response.body).to.have.property('lastname', accountMock1.lastname);
                    expect(response.body).to.have.property('firstname', accountMock1.firstname);
                    expect(response.body).to.have.property('email', accountMock1.email);
                    expect(response.body).to.have.property('admin', accountMock1.admin);
                    expect(response.body).to.have.property('active', accountMock1.active);
                    expect(response.body).to.have.property('session_id', accountMock1.session_id);
                    expect(response.body).to.have.property('creation_date', accountMock1.creation_date);
                    expect(response.body).to.have.property('current_connexion_date', accountMock1.current_connexion_date);
                    expect(response.body).to.have.property('last_connexion_date', accountMock1.last_connexion_date);
                    expect(response.body).to.have.property('token', accountMock1.token);
                    done();
                });
        } catch (exception) {
            console.log('exception: %s', exception.stack);
